...
const userController = {
  ...
  quickCreateDoc: async (req, res) => {
    ...
    try {
      ...
      if (!gender) emptyFields.push("Gender");
      if (!dateOfBirth) emptyFields.push("Birthday");
      if (!defaultPassword) emptyFields.push("Default Password");
      ...
      let genPassword = "";
      if (defaultPassword === "default") {
        genPassword = "portal01";
      }
      if (defaultPassword === "generated") {
        genPassword = generateCredential.password(10);
      }
  ...
  adminResetPass: async (req, res) => {
    try {
      ...
      if (update) {
        ...
        return res.status(200).json({ message: "Password reset error!" });
      }
    } catch (error) {
      console.log(
