      ...
      renderCell: (params) => {
        return (
          <Box display="flex" gap={2}>
            <Link
              to={`/member/curriculums/${currYear}/${courseYear}/${params?.value}`}
              style={{
                alignItems: "center",
                textDecoration: "none",
              }}
        ...
        return (
          <Box sx={{ display: "flex", gap: 2 }}>
            <ButtonBase
              onClick={() => {
                navigate(
                  `/member/feedbacks/${currYear}/${courseYear}/${params?.row?.currVer}/${params?.row?.currVerID}`
                );
              }}
            >
              ...
              onClick={() => {
                setValidateDialog({
                  ...
                  onConfirm: () => {
                    setConfirmDialog({
                      isOpen: true,
                      title: `Are you sure to submit for review curriculum ${params?.row?.currVerID}? `,
                      onConfirm: () => {
          ...
          <>
            <Box sx={{ display: "flex", gap: 2 }}>
              <ButtonBase
                ...
                onClick={(event) => {
                  navigate(`/member/generate/${params?.row.currVerID}`);
                }}
              >
                <Paper_Icon
  ...
  const toggleStatus = async ({ val }) => {
    ...
    try {
      ...
      const apiCurrVerStatus = await axiosPrivate.patch(
        ...
        JSON.stringify({ status: "chairReview" })
      );
