...
const EditSubjectDialogue = (props) => {
  const { auth } = useAuth();
  ...
  const { editSubjectDialogue, setEditSubjectDialogue } = props;
  const handleClose = (event, reason) => {
    if (reason && reason === "backdropClick")
      return (
        clearItems(),
        setEditSubjectDialogue({ ...editSubjectDialogue, isOpen: false })
      );
  };
    ...
    if (!subjectCodeError) {
      try {
        const response = await axios.patch(
          `/api/subject/update/${subjectCode}`,
          doc,
          {
        ...
        if (response.status === 200) {
          ...
          if (response.status === 200) {
            ...
            editSubjectDialogue.onConfirm();
          }
          setEditSubjectDialogue({ isOpen: false });
          clearItems();
        }
  ...
  return (
    <>
      ...
      <Dialog
        sx={{ textAlign: "center" }}
        open={editSubjectDialogue.isOpen}
        onClose={handleClose}
      >
            ...
            <Box display="flex" flexDirection="column" alignItems="center">
              ...
              <Typography variant="h3">
                ...
                <span style={{ textTransform: "uppercase" }}>
                  {editSubjectDialogue?.subjectCode}
                </span>
              </Typography>
            </Box>
          </DialogTitle>
...
export default EditSubjectDialogue;
